...
当前已完成 P0 至 P7 的代码与基础设施范围，包括身份安全、数据生命周期、可观测性/性能/成本、拟人评测，以及生产镜像与发布供应链；后续增强已完成多模态模型输入、多智能体管理、渠道归属隔离、智能体安全生命周期、外部身份/会话映射、入站消息到智能体运行的可靠闭环、高精度反思与关系校准、长对话分层摘要与上下文压缩、多模型同源回放与并列质量对比、运营总览、用户身份可见性与管理会话治理、用户访问策略与可信角色覆盖，以及 Telegram 安全出站闭环。生产式 Docker Compose、全链迁移、服务闭环和隔离备份恢复已由 Linux CI 实跑通过；仅 GitHub `main` 分支保护因私有仓库当前套餐限制而待处理。最新开发计划见 [`docs/plans/2026-09-11-v15.md`](docs/plans/2026-09-11-v15.md)。
