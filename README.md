...
- 拟人质量趋势、同源基线差异、不可变人工决策与职责分离审批：按完整冻结快照和独立样本门槛比较候选与基线，将聚合证据、受控结论和操作者元数据冻结为无正文 JSON 原始字节；审批使用 Agent 作用域 Ed25519 密钥生成可独立复核的签名证明，并可选关联只读发布变更，不会自动调参或发布。
...
- Alembic 配置、对话、附件、多模态消息块、加密密钥、审计、认知运行、长期记忆、可靠异步任务、渠道控制平面、OIDC、数据生命周期、可观测成本、拟人评测、多模型对比、评测决策与审批、多智能体管理、渠道归属、智能体生命周期、外部入站路由、用户访问策略与告警运营历史迁移，当前唯一 head 为 `20260917_0037`。
...
管理人员可把同源候选与基线的冻结聚合证据记录为不可变人工决策，再由另一名持有 `evaluation:approve` 权限的管理员批准或驳回。每个决策只允许一份终态审批，禁止自审；等待更多证据的决策只能驳回。审批可关联环境和变更编号作为只读治理事实，但不会修改配置、调用发布系统或授予自动执行能力。

审批证明使用 Agent 作用域 SecretStore 中的 32 字节 Ed25519 私钥签署 canonical JSON，仅公开算法、key ID、公钥、签名和 SHA-256。管理后台可执行服务端验签并下载原始证明；密钥轮换后历史证明仍可通过证明内公钥验证，私钥不会通过 API、日志、审计、导出或配置差异返回。
