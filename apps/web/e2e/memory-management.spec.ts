...
test('可以创建、确认和遗忘带来源的长期记忆', async ({ page }) => {
  ...
  await expect(page.getByText('用户私有', { exact: true }).last()).toBeVisible()
  await expect(page.getByText('普通', { exact: true }).last()).toBeVisible()
  await expect(page.getByText('管理导入', { exact: true })).toBeVisible()
...
test('只读角色不能看到记忆写操作入口', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '向量索引重建任务' })).toBeVisible()
