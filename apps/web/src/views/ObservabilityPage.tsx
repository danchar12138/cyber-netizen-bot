  ...
  return (
    <div className="page">
      ...
      <section className="metric-grid" aria-label="服务等级与成本指标">
        ...
        <article className="metric-card"><div className="metric-icon"><ShieldCheck size={18} /></div><p>Agent Run 成功率</p><strong>{data ? `${data.agent_runs.success_rate_percent.toFixed(2)}%` : '—'}</strong><span>{data?.agent_runs.completed_runs ?? 0} 成功 · {data?.agent_runs.unsuccessful_runs ?? 0} 未成功</span></article>
      ...
      {trace.data && <div className="trace-grid">
        ...
        <section className="panel model-invocations"><div className="panel-heading"><h2>模型尝试</h2><span className="subtle">{trace.data.model_invocations.length} 次</span></div>{trace.data.model_invocations.map((item) => <article key={`${item.purpose}-${item.attempt}`}><strong>{item.provider} / {item.model}</strong><span>{item.status} · 尝试 {item.attempt} · {item.latency_ms ?? 0} ms · {formatUsd(item.estimated_cost_microusd)}</span><small>用途 {item.purpose} · 输入 {item.input_tokens ?? '—'} · 输出 {item.output_tokens ?? '—'}</small></article>)}</section>
      </div>}
    </div>
  )
}
