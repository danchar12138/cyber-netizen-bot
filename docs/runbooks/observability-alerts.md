...
1. “候选阈值回放”只读取校准窗口内的冻结建议反馈和当前 Agent 的生命周期聚合事实（持续时间、出现次数、状态）；不读取或生成原始正文、任务载荷、Prompt、隐藏推理或 Secret。
2. 回放将当前阈值与服务端校准提案逐条比较，展示当前/候选触发数量、避免数量、保留数量、保留反馈结论和显式替代动作分布；生命周期事实缺失会单独计数，不进行猜测。
3. 回放是有界场景模拟，不代表真实反事实实验，不证明阈值与恢复、业务结果或人工行为存在因果关系。结果只读，不自动执行、不自动调参、不自动发布。
4. 历史反馈没有替代动作时显示为“无显式标签”；替代动作仅允许 `acknowledge`、`suppress`、`observe`，由服务端校验并按租户/Agent 隔离。
